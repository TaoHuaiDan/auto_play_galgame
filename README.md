...
8. **进入无人值守循环。** 冒烟测试通过后调用 `play_until_choice`。它在本地
   捕获、OCR、解析、去重、记录和推进，只在选项、批次上限或安全边界出现时把
   汇总 JSON 返回给 Codex。返回 `dialogue_not_detected`、
...
限制到边界，类型错误会直接报错。传 `configure_game_timing(profile={})` 可清除
...
MCP 会用请求时的事件数量、序号范围和 SHA-256 校验原始前缀没有变化，先把总结写入 `compactions/segment_*.json`，再从活动会话中清除这段原始事件；校验失败时不会删除任何数据。省流文件保留剧情摘要、人物、选项与实际决定、路线变量、未解决伏笔、重要短句和 OCR 不确定性，后续 `get_codex_context` 会把它与新产生的 JSON 事件一起返回。未解决选项会保护其原始事件，不会被压缩切断。该流程只由 Codex 负责语义总结，MCP 负责阈值、完整性校验和可恢复落盘；不会把未经模型确认的摘要当成事实。
...
```text
play_until_choice(max_steps=40, background=true, wait_seconds=0.05, transition_wait_seconds=1.2, max_batch_chars=6000)
```
